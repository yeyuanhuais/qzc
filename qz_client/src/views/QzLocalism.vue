	...
	<div class="situation">
		<div class="mainHead">
			...
			<el-breadcrumb separator-class="el-icon-arrow-right">
				...
				<el-breadcrumb-item>闽南语</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
		<div class="mainBody">
			<div class="box">
				<div class="listDiv" v-for="item in localisms" :key="item.id" v-show="item.isShow">
					<p>
						<router-link :to="'/qzLocalism/'+item.id" :title="item.title">{{item.title|ellipsis}}</router-link>
						<span>{{item.time}}&nbsp;&nbsp;{{item.source}}</span>
					</p>
					<div>
						<router-link :to="'/qzLocalism/'+item.id" class="left">
							<img v-if="item.imagePath" :src="item.imagePath" />
							<img v-else src="../assets/images/洛阳桥.jpg" />
						</router-link>
						<router-link :to="'/qzLocalism/'+item.id" class="righta">
							<span
								class="right"
								v-html="msg(item.content).length > 140 ? msg(item.content).slice(0, 140) + '...' :msg(item.content)"
							></span>
						</router-link>
					</div>
				</div>
...
export default {
	...
	computed: {
		...mapState(['localisms', 'articleCount'])
	},

	mounted() {
		this.$store.dispatch('getLocalismList', this.query)
	},
	created() {},
	methods: {
